...
function useMetaMaskEthersSignerInternal(parameters: { initialMockChains?: Readonly<Record<number, string>> }): UseMetaMaskEthersSignerState {
  ...
  const [ethersReadonlyProvider, setEthersReadonlyProvider] = useState<ethers.Provider | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    // Prefer a read-only provider that does NOT go through MetaMask to avoid surfacing RPC errors in inpage.js
    let rop: ethers.Provider = bp;
    const rpcUrlFromMock: string | undefined = initialMockChains?.[chainId];
    const fallbackRpcUrl = chainId === 11155111 ? "https://ethereum-sepolia-rpc.publicnode.com" : (chainId === 31337 ? "http://localhost:8545" : undefined);
    const finalRpcUrl = rpcUrlFromMock || fallbackRpcUrl;
    if (finalRpcUrl) { rop = new ethers.JsonRpcProvider(finalRpcUrl); }
