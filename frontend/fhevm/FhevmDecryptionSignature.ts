...
class FhevmDecryptionSignatureStorageKey {
  ...
  constructor(instance: FhevmInstance, contractAddresses: string[], userAddress: string, publicKey?: string) {
    ...
    const hash = ethers.TypedDataEncoder.hash(empty.domain, { UserDecryptRequestVerification: empty.types.UserDecryptRequestVerification }, empty.message);
...
export class FhevmDecryptionSignature {
  ...
  private constructor(parameters: any) {
  ...
  static fromJSON(json: unknown) { const data = typeof json === "string" ? JSON.parse(json) : json; return new FhevmDecryptionSignature(data); }
  ...
  static async new(instance: FhevmInstance, contractAddresses: string[], publicKey: string, privateKey: string, signer: ethers.Signer): Promise<FhevmDecryptionSignature | null> {
    try {
      ...
      return new FhevmDecryptionSignature({ publicKey, privateKey, contractAddresses: contractAddresses as `0x${string}`[], startTimestamp, durationDays, signature, eip712, userAddress });
    } catch { return null; }
  }
